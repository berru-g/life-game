//myDiv function startFunction() { ///start var x = document.getElementById("myDIV"); if (x.style.display === "none") { x.style.display = "block"; } else { x.style.display = "none"; } } //démo //algo indé te souhaite une bonne journée suivant l'heure
if (new Date().getHours() > 17) { document.getElementById("demo").innerHTML = "Bonsoir et bienvenue dans le 'Life Game'.";
    window.style("background-color:black"); } //time function startTime() { var today = new Date(); var h = today.getHours(); var m
= today.getMinutes();
...
function checkTime(i) {
    if (i < 10) { i = "0" + i }; // add zero
    in front of numbers < 10
    return i;
} //function perso1 function myDealer() { var txt; var person=c onfirm(
" Salut tu veut de la weed?  Confirme et gagne: 1 points de détente et perds: 1 points de santé. Passe ton chemin et gagne 1 points de Karma et 1 de Santé!"); //var "nom" dans l 'alert
if (person == null || person == "") {
    ...
    txt = "Accepter! Tu gagne: 1 points de détente, Tu perds: 1 points de santé "; //dans var "text ""(pour aligner sur une deuxieme ligne faire "premier mess .n second message .n trois etc") //comment suprimer un point? /=o u -=m 'affiche "NaN" car valeur non numerique?
...
function myflic() {
    ...
    // var gagnant;
    var txt;
    player = window.prompt("Viens on joues à Chifoumi! pierre feuille ou ciseau ?"); //var "nom" dans l'alert player=p layer.toLowerCase(); random=M ath.random(); // 1/3 de chance pour chacun if (random < 1 / 3) { computer='pierre' ; } else
    if (random < 2 / 3) { computer = 'feuille'; } else { computer = 'ciseau'; } //definir une égalité if (computer==p layer) { alert( "egalité, gagne un point détente !"); document.getElementById( "détente").innerHTML +="🍃" ; } else if ((computer=='pierre'
    &&
    player == 'ciseau') || (computer == 'ciseau' && player == 'feuille') || (computer == 'feuille' && player == 'pierre')) {
    alert("Perdu. Déguage petit et gagne un point expérience !"); //solution n 'ont concluante, car je souhaite afficher le result dans "txt"
